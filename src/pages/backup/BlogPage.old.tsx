...
import { Wrench, Menu, X, Timer, LogIn, Eye, LogOut, Settings, Wallet, Info, Search, ShieldCheck, Plus, NotebookPen, Lock } from 'lucide-react';
...
export default function BlogPage() {
  ...
  const loadPosts = useCallback(async (f: typeof filters, pageNum = 1, append = false) => {
    try {
      const res = await fetchBlogPosts({ ...f, page: pageNum, limit: 24 });
      setPosts(prev => append ? [...prev, ...res.posts] : res.posts);
      setHasMore(res.hasMore);
      setPage(pageNum);
      setBlogPrivate(false);
    } catch (err: any) {
      if (err.message === 'This blog is private') {
        setBlogPrivate(true);
      } else {
  ...
  // Fetch archive once on mount (it doesn't depend on filters)
  useEffect(() => {
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b border-border bg-card/50 sticky top-0 z-30 backdrop-blur-sm">
        ...
        <div className="container max-w-6xl px-4 py-3 flex items-center gap-3">
          ...
          {/* Log in button — shown when not authenticated */}
          ...
          {/* Nav icons — fixed order: Blog · Expenses · Tracker */}
          <span title="Build Blog" className="inline-flex items-center justify-center w-8 h-8 rounded-md text-primary bg-primary/10 shrink-0">
            <NotebookPen className="w-4 h-4" />
          </span>
          {isAuthenticated && (
            <Link to="/expenses" title="Expenses" className="inline-flex items-center justify-center w-8 h-8 rounded-md text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors shrink-0">
              <Wallet className="w-4 h-4" />
            </Link>
          )}
          {isAuthenticated && (
            <Link to="/tracker" title="Build Tracker" className="inline-flex items-center justify-center w-8 h-8 rounded-md text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors shrink-0">
          ...
          <div className="flex-1 min-w-0 space-y-4">
            {blogPrivate ? (
              <div className="text-center py-20 text-muted-foreground">
                ...
                <p className="text-lg font-medium">This blog is private</p>
                <p className="text-sm mt-1">It appears someone doesn't want to share their rivets.</p>
                {!isAuthenticated && !demoMode && (
                  <Link
                    to="/login"
                    state={{ from: '/blog' }}
                    ...
                    {hasMore && !searchQuery.trim() && (
                      <div className="text-center pt-2">
                        <button
                          ...
                          className="inline-flex items-center gap-2 px-6 py-2.5 text-sm font-medium rounded-lg border border-border bg-card hover:bg-secondary text-foreground transition-colors disabled:opacity-50"
                        >
      ...
      {(() => {
        ...
        const base = `https://${parts.slice(1).join('.')}`;
        return (
          <footer className="border-t border-border/50 text-center text-xs text-muted-foreground py-4 mt-6">
            <p className="flex items-center justify-center gap-4">
              <a href={`${base}/impressum`} target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors">Impressum</a>
              <a href={`${base}/datenschutz`} target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors">Datenschutzerklärung</a>
              <a href={`${base}/tos`} target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors">Terms of Service</a>
            </p>
          </footer>
        );
      })()}
