 ...
 * reproducible.
 */
function buildMst(nodeIds: string[], pos: Map<string, Point>): MstEdge[] {
...
export function deriveHarness(
  ...
  function buildTree(tree: HarnessTree): void {
    ...
    // 3b. MST over the node positions — the connectivity backbone.
    const sortedNodeIds = tree.nodeIds.slice().sort();
    const mstEdges = buildMst(sortedNodeIds, pos);
